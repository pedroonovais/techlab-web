# 🧪 TechLab — Sistema de Gerenciamento de Pátios, IoT e Motos

Aplicação **web** (Thymeleaf) e **API REST** construída em **Spring Boot 3** para administrar pátios, dispositivos **IoT** e **motos**, com autenticação (OAuth2 + formulário + JWT), **dashboard** com KPIs e UI dark usando **TailwindCSS** (CDN).

---

## 👥 Integrantes

- **Davi Alves de Lima** — RM 556008  
- **Pedro Henrique Mendonça de Novais** — RM 555276  
- **Rodrigo Alcides Bohac Ríos** — RM 554826  

---

## 📦 Estrutura do Projeto

- **Build**: Gradle (Java 17)  
- **Main class**: `com.techlab.Application`  
- **View engine**: Thymeleaf  
- **UI**: TailwindCSS via CDN (ver `templates/fragments/header.html`)  
- **Banco**: Oracle Database (Flyway para migrações)  
- **Auth**: Spring Security (Form login + OAuth2 GitHub/Google + JWT para API REST)  

### Pacotes principais (em `src/main/java/com/techlab/`):

```
config/                 # Configurações (SecurityConfiguration, JwtAuthenticationFilter, RestExceptionHandler)
dashboard/              # Dashboard com KPIs (DashboardController)
iot/                    # CRUD de IoT (Iot, IotController, IotRestController, IotRepository, IotService, IotDTO)
moto/                   # CRUD de Moto (Moto, MotoController, MotoRepository, MotoService, MotoDTO)
patio/                  # CRUD de Pátio (Patio, PatioController, PatioRepository, PatioService, PatioDTO)
auth/                   # Autenticação (AuthController, AuthRestController, JwtService, LoginForm, RegisterForm)
user/                   # Gerenciamento de usuários (User, UserController, UserService, UserDetailsServiceImpl)
helper/                # Classes base e utilitários (BaseController, BaseService)
debug/                  # Endpoints de debug (DebugController)
```

**Templates** (em `src/main/resources/templates/`): `index.html`, `login.html`, `logout.html`, `register.html`, pastas `patio/`, `iot/`, `moto/`, `fragments/` (header, footer), etc.

**Migrações Flyway** (em `src/main/resources/db/migration/`): 
- `V1__create_table_user.sql` — Tabela de usuários
- `V2__create_table_patio.sql` — Tabela de pátios
- `V3__create_table_iot.sql` — Tabela de dispositivos IoT
- `V4__create_table_moto.sql` — Tabela de motos com relacionamentos

**Migrações de desenvolvimento** (em `src/main/resources/db/migration-dev/`): Dados iniciais para testes.

---

## 🔧 Tecnologias

- **Spring Boot 3.5.4**
- Spring MVC, Spring Security, Spring Data JPA
- **Thymeleaf** (templates web)
- **TailwindCSS** (CDN)
- **Oracle Database** + **Flyway** (migrações)
- **JWT** (JSON Web Tokens) para autenticação de API REST
- **Lombok** (redução de boilerplate)
- **Docker** (Dockerfile e compose.yaml)
- **Gradle** (build tool)

---

## 🚀 Como Rodar

### Pré‑requisitos
- **Java 17+**
- **Oracle Database** acessível (local ou remoto)
- **Gradle** (wrapper incluso: `./gradlew` ou `gradlew.bat`)

### 1) Configurar Banco de Dados

O projeto está configurado para usar **Oracle Database**. No modo de desenvolvimento, a aplicação usa as credenciais configuradas em `application.properties`:

```properties
spring.datasource.url=jdbc:oracle:thin:@oracle.fiap.com.br:1521:orcl
spring.datasource.username=rmXXXXXX
spring.datasource.password=000000
```

Para produção, configure as variáveis de ambiente:

```bash
DB_URL=jdbc:oracle:thin:@seu-host:1521:orcl
DB_USER=seu_usuario
DB_PASS=sua_senha
```

### 2) Configurar variáveis de ambiente (opcional para OAuth2)

Para habilitar OAuth2 (GitHub/Google), configure as seguintes variáveis:

```bash
# OAuth2 (opcional)
GITHUB_CLIENT_ID=xxxxx
GITHUB_CLIENT_SECRET=xxxxx
GOOGLE_CLIENT_ID=xxxxx
GOOGLE_CLIENT_SECRET=xxxxx

# Perfil ativo (padrão: dev)
ACTIVE_PROFILE=dev  # ou 'prod' para produção
```

> O perfil ativo é definido por `ACTIVE_PROFILE` (padrão: `dev`). No `dev` as migrações do Flyway são procuradas em `classpath:db/migration,classpath:db/migration-dev` (ver `application.properties`).

### 3) Subir a aplicação

```bash
# Linux/Mac
./gradlew bootRun

# Windows
gradlew.bat bootRun
```

Acesse **http://localhost:8080**.

### 4) Build de JAR

```bash
./gradlew bootJar
java -jar build/libs/*.jar
```

### 5) Docker (opcional)

Uma imagem pode ser gerada usando o `Dockerfile` incluso:

```bash
docker build -t techlab:latest .
docker run -p 8080:8080 --name techlab \
  -e ACTIVE_PROFILE=prod \
  -e DB_URL=jdbc:oracle:thin:@host.docker.internal:1521:orcl \
  -e DB_USER=seu_usuario \
  -e DB_PASS=sua_senha \
  techlab:latest
```

### 6) Docker Compose (PostgreSQL para desenvolvimento local)

O projeto inclui um `compose.yaml` com PostgreSQL para desenvolvimento local:

```bash
docker-compose up -d
```

---

## 🗂️ Banco de Dados (Modelo de Dados)

### Tabelas principais:

```sql
-- Usuários
CREATE TABLE techlabuser (
    id         NUMBER(19) GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    email      VARCHAR2(255) UNIQUE NOT NULL,
    name       VARCHAR2(255),
    avatar_url VARCHAR2(255),
    password   VARCHAR2(255)
);

-- Pátios
CREATE TABLE patio (
    id   NUMBER(19) GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    nome VARCHAR2(255) NOT NULL UNIQUE
);

-- Dispositivos IoT
CREATE TABLE iot (
    id           NUMBER(19) GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    ativo        NUMBER(1) NOT NULL,
    bateria      NUMBER(10),
    coordenada_x VARCHAR2(255) NOT NULL,
    coordenada_y VARCHAR2(255) NOT NULL
);

-- Motos (com relacionamentos)
CREATE TABLE moto (
    id           NUMBER(19) GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    modelo       VARCHAR2(255) NOT NULL,
    placa        VARCHAR2(255) NOT NULL UNIQUE,
    data_entrada TIMESTAMP NOT NULL,
    data_saida   TIMESTAMP,
    iot_id       NUMBER(19),
    patio_id     NUMBER(19),
    CONSTRAINT fk_moto_iot FOREIGN KEY (iot_id) REFERENCES iot(id),
    CONSTRAINT fk_moto_patio FOREIGN KEY (patio_id) REFERENCES patio(id)
);
```

### Relacionamentos:

- **Moto** → **IoT** (Many-to-One): Uma moto pode ter um dispositivo IoT associado
- **Moto** → **Patio** (Many-to-One): Uma moto está em um pátio

---

## 🌐 Rotas Principais

### Autenticação Web

- `GET /login` — página de login  
- `GET /register` — cadastro de usuário  
- `POST /register` — cria o usuário  
- `POST /logout` — encerra sessão  
- OAuth2: `GET /oauth2/authorization/github` e `GET /oauth2/authorization/google`

### API REST (JSON)

#### Autenticação
- `POST /api/auth/login` — Login via API retornando token JWT
  ```json
  {
    "email": "usuario@example.com",
    "password": "senha123"
  }
  ```
  Resposta:
  ```json
  {
    "token": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..."
  }
  ```

#### Dispositivos IoT
- `PUT /api/iot/{id}` — Atualiza um dispositivo IoT (requer autenticação JWT)
  ```json
  {
    "ativo": true,
    "bateria": 85,
    "coordenadaX": "10.5",
    "coordenadaY": "20.3"
  }
  ```

#### Usuário
- `GET /api/user` — Retorna dados do usuário autenticado

### Dashboard

- `GET /` — redirect para `/index`
- `GET /index` — Dashboard com KPIs:
  - Total de Pátios
  - Total de IoTs (ativos/inativos)
  - Total de Motos
  - Média de bateria dos dispositivos IoT
  - Distribuição de bateria por faixas (0-20, 21-40, 41-60, 61-80, 81-100)

### Pátio (Web)

- `GET /patio` — listar pátios
- `GET /patio/form` — formulário de criação
- `POST /patio/form` — criar/atualizar (envia `id` para editar)
- `GET /patio/{id}/edit` — editar pátio
- `POST /patio/{id}/delete` — excluir pátio

### IoT (Web)

- `GET /iot` — listar dispositivos IoT
- `GET /iot/form` — formulário de criação
- `POST /iot/form` — criar/atualizar
- `GET /iot/{id}/edit` — editar dispositivo
- `POST /iot/{id}/delete` — excluir dispositivo

### Moto (Web)

- `GET /moto` — listar motos
- `GET /moto/form` — formulário de criação
- `POST /moto/form` — criar/atualizar
- `GET /moto/{id}/edit` — editar moto
- `POST /moto/{id}/delete` — excluir moto

### Debug

- `GET /debug/test-user-creation` — Testa criação de usuário
- `GET /debug/check-user` — Verifica usuário existente

---

## 🎨 UI / Padrão Visual

- Tema **dark** com **TailwindCSS** via CDN (Inter como font)  
- Componentização de **navbar** e **footer** em `templates/fragments/`  
- Páginas seguem o padrão: **bgs** `gray-900/800`, **borders** `gray-700`, **primárias** `green-600/700`, **destaques** `cyan-500`  
- Interface responsiva e moderna

---

## 🔐 Segurança

### Autenticação Web

- Spring Security com **form login** e **OAuth2** (GitHub/Google).  
- `SecurityConfiguration` define regras públicas/privadas e provedor com **BCrypt**.  
- CSRF habilitado e utilizado nos formulários (`_csrf`).

### Autenticação API REST

- **JWT (JSON Web Tokens)** para autenticação de API REST
- Filtro `JwtAuthenticationFilter` valida tokens JWT nas requisições `/api/**`
- Endpoint `/api/auth/login` retorna token JWT após autenticação bem-sucedida
- Tokens devem ser enviados no header: `Authorization: Bearer <token>`

### Rotas Públicas

- `/login`, `/register`, `/debug/**`, `/css/**`, `/js/**`, `/images/**`
- `/api/auth/login` (endpoint de login da API)

### Rotas Protegidas

- Todas as outras rotas requerem autenticação (web ou JWT)

---

## 📊 Dashboard e KPIs

O dashboard (`/index`) exibe:

- **Total de Pátios**: Contagem total de pátios cadastrados
- **Total de IoTs**: Contagem total de dispositivos IoT
- **IoTs Ativos**: Quantidade de dispositivos IoT ativos
- **IoTs Inativos**: Quantidade de dispositivos IoT inativos
- **Total de Motos**: Contagem total de motos cadastradas
- **Média de Bateria**: Média da bateria de todos os dispositivos IoT
- **Distribuição de Bateria**: Contagem de dispositivos por faixa de bateria:
  - 0-20%
  - 21-40%
  - 41-60%
  - 61-80%
  - 81-100%

---

## 📄 Licença

Projeto acadêmico — FIAP. Uso educacional.
